...
def main(session: snowpark.Session): 
    ...
    table_name = 'ALERT_FORECAST_PROPHET_MODEL_DATA'
    sql_stmt = f'SELECT * FROM {table_name} ORDER BY "y" DESC'
    ...
    # mae = np.mean(np.abs(forecast['y'] - forecast['yhat']))

    # # Calculate the Mean Squared Error (MSE)
    # mse = np.mean((forecast['y'] - forecast['yhat'])**2)
